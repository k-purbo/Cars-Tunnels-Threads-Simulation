	...
	public final void run() {
		...
		while(true) {
			for(Tunnel tunnel : tunnels) {
				if (tunnel instanceof PreemptivePriorityScheduler || tunnel instanceof PriorityScheduler) {
					tunnel.tryToEnterInner(this);
					doWhileInTunnel();
					tunnel.exitTunnelInner(this);
					return;
				}
				else {
	...
	public final void doWhileInTunnel() {    	
		...
		try {
			if (!(this instanceof Ambulance)) {
				long totalTime = (10 - speed) * 100;
				long remainingTime = totalTime;
				long startTime = System.currentTimeMillis();	
				while(condition.await(remainingTime, TimeUnit.MILLISECONDS)) {
					long endTime = System.currentTimeMillis();	
					long timeElapsed = endTime - startTime;
					remainingTime = remainingTime - timeElapsed;
					//if ambulance signaled its entrance
				...
				//signalAll as ambulance enters and leaves tunnel
				//signal in here to avoid requiring the lock again ins scheduler
				condition.signalAll();
